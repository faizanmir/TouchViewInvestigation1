...
import com.example.touchviewinvestigation.callback.FilePickerCallback
import com.example.touchviewinvestigation.callback.FileReceiverCallback
import com.example.touchviewinvestigation.callback.FolderPickerCallback
import com.example.touchviewinvestigation.callback.StorageAccessCallback
    ...
    private fun intentToDocumentFiles(intent: Intent?): List<DocumentFile> {
        ...
        return uris.mapNotNull { uri ->
            if (uri.isDownloadsDocument && Build.VERSION.SDK_INT < 28 && uri.path?.startsWith("/document/raw:") == true) {
                ...
                if (Build.VERSION.SDK_INT < 21 && file.getStorageId(context).matches(
    ...
    companion object {
        ...
        fun getDefaultExternalStorageIntent(context: Context): Intent {
            return Intent(Intent.ACTION_OPEN_DOCUMENT_TREE).apply {
                if (Build.VERSION.SDK_INT >= 26) {
                    putExtra(DocumentsContract.EXTRA_INITIAL_URI, context.fromTreeUri(
                        DocumentFileCompat.createDocumentUri(PRIMARY))?.uri)
                }
            }
        }
