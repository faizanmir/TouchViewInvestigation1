...
import com.example.touchviewinvestigation.SimpleStorage.Companion.KITKAT_SD_CARD_ID
import com.example.touchviewinvestigation.SimpleStorage.Companion.KITKAT_SD_CARD_PATH
import com.example.touchviewinvestigation.StorageId.DATA
import com.example.touchviewinvestigation.StorageId.HOME
import com.example.touchviewinvestigation.StorageId.KITKAT_SDCARD
import com.example.touchviewinvestigation.StorageId.PRIMARY
import com.example.touchviewinvestigation.extension.fromSingleUri
...
import com.example.touchviewinvestigation.extension.replaceCompletely
...
object DocumentFileCompat {
    ...
    * API 30+     => content://com.android.providers.downloads.documents/document/msf%3A42
    ...
    fun fromUri(context: Context, uri: Uri): DocumentFile? {
        return when {
            ...
            uri.isTreeDocumentFile -> context.fromTreeUri(uri)?.run { if (isDownloadsDocument) toWritableDownloadsDocumentFile(context) else this }
                ...
                if (Build.VERSION.SDK_INT < 21 && it.getStorageId(context).matches(
                        SD_CARD_STORAGE_ID_REGEX
                    )) {
    ...
    @JvmStatic
    fun isDownloadsUriPermissionGranted(context: Context) = isUriPermissionGranted(context, Uri.parse(
        DOWNLOADS_TREE_URI
    ))

    @JvmStatic
    fun isDocumentsUriPermissionGranted(context: Context) = isUriPermissionGranted(context, Uri.parse(
        DOCUMENTS_TREE_URI
    ))
